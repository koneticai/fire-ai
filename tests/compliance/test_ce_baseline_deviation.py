...
from src.app.models.ce_test import CETestSession, CETestMeasurement, CETestDeviation
...
async def test_baseline_establishment_first_inspection(db_session, mock_building):
    ...
    measurements = {"pressure": 50.0, "velocity": 2.5, "force": 45.0}
...
async def test_deviation_detection_normal(db_session, mock_building):
    ...
    # Mock baseline
    baseline = {"pressure": {"value": 50.0, "unit": "Pa"}}
...
@pytest.mark.asyncio
async def test_deviation_detection_critical_with_notification(
    db_session, mock_building
):
    ...
    with (
        patch.object(baseline_service, "get_baseline", return_value=baseline),
        patch(
            "src.app.services.ce_deviation_analyzer.notification_service.notify_critical_defect",
            mock_notify,
        ),
    ):
        # Run analyzer
...
@pytest.mark.asyncio
async def test_mixed_severity_deviations(db_session, mock_building):
    """Test session with normal, warning, and critical deviations."""
    building, owner = mock_building

    # Create test session with mixed severity measurements
    session_id = uuid4()
    session = CETestSession(
        id=session_id,
        building_id=building.id,
        created_by=owner.id,
        session_name="C&E Test Mixed Severity",
        status="active",
        test_configuration={},
        measurements=[
            CETestMeasurement(
                ...
                measurement_type="pressure_differential",
                location_id="floor_1",
                measurement_value=52.0,  # +4% from baseline (50.0) - NORMAL, no record
                unit="Pa",
                timestamp=datetime.now(timezone.utc),
            ),
            CETestMeasurement(
                ...
                measurement_type="air_velocity",
                location_id="exit_1",
                measurement_value=2.8,  # +12% from baseline (2.5) - WARNING/MAJOR
                unit="m/s",
                timestamp=datetime.now(timezone.utc),
            ),
            CETestMeasurement(
                ...
                measurement_type="door_force",
                location_id="door_1",
                measurement_value=30.0,  # -33% from baseline (45.0) - CRITICAL
                unit="N",
                timestamp=datetime.now(timezone.utc),
            ),
        ],
    ...
    # Verify only 2 deviations created (warning + critical, normal ignored)
    assert len(added_deviations) == 2

    # Verify severity distribution
    severity_counts = {}
    for deviation in added_deviations:
        severity_counts[deviation.severity] = (
            severity_counts.get(deviation.severity, 0) + 1
        )

    assert severity_counts.get("major", 0) == 1  # velocity warning
    assert severity_counts.get("critical", 0) == 1  # force critical

    # Verify deviation types
    deviation_types = {d.deviation_type for d in added_deviations}
    assert "velocity_baseline_deviation" in deviation_types
    assert "force_baseline_deviation" in deviation_types
    assert (
        "pressure_baseline_deviation" not in deviation_types
    )  # Normal deviation ignored

    # Verify notification triggered only once (for critical deviation)
    assert mock_notify.call_count == 1
    call_args = mock_notify.call_args
    defect_details = call_args[1]["defect_details"]
    assert "force" in defect_details["technical_details"]  # Only critical parameter
    assert (
        "velocity" not in defect_details["technical_details"]
    )  # Warning doesn't trigger notification
