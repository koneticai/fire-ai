...
func handleEvidence(w http.ResponseWriter, r *http.Request) {
	...
	_, err = db.Exec(ctx, query, evidenceID, sessionID, evidenceType,
...
func handleCRDTResults(w http.ResponseWriter, r *http.Request) {
	...
	err = db.QueryRow(ctx, query, sessionID).Scan(&sessionDataJSON, &vectorClockJSON)
	...
	_, err = db.Exec(ctx, updateQuery, sessionID, string(mergedDataJSON), string(mergedVectorClockJSON))
	if err != nil {
